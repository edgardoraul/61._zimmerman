...
function process_si_contact_form()
{
	...
	if ( $_SERVER['REQUEST_METHOD'] == 'POST' && @$_POST['do'] == 'contact' )
	{
		...
		if ( isset( $GLOBALS['DEBUG_MODE'] ) && $GLOBALS['DEBUG_MODE'] == false )
		{
			...
			{
				$errors['message_error'] = __('Tienes que ingresar un mensaje.', 'webmoderna');
			}
		}
...
<?php endif; ?>
							...
							<form method="post" action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI'] . $_SERVER['QUERY_STRING']);?>" id="contact_form">
								...
								<input  type="text" placeholder="<?php _e('Apellido y Nombre', 'webmoderna');?>" maxlength="50" name="ct_name" value="<?php echo htmlspecialchars(@$_SESSION['ctform']['ct_name']);?>" />
								<input  type="hidden" name="do" value="contact" />

								<div class="resultado"><?php echo @$_SESSION['ctform']['email_error'];?></div>
								<input  type="email" name="ct_email" id="ct_email" placeholder="E-Mail" maxlength="40" value="<?php echo htmlspecialchars(@$_SESSION['ctform']['ct_email']) ?>" />
