...
class DmConfirmEmail_Models_RegisterForm {
	...
	public function formRegister() {
		...
		login_header( __('Formulario de registro', 'emyth' ), '<p class="message register">' . __('Registrar para este sitio', 'emyth' ) . '</p>', $errors);
		?>

		<form name="registerform" id="registerform" action="<?php echo esc_url( site_url('wp-login.php?action=register', 'login_post') ); ?>" method="post">
			<p>
				<label for="user_login"><?php _e( 'Nombre de usuario', 'emyth' );?><br />
					<input type="text" name="user_login" id="user_login" class="input" value="<?php echo esc_attr(wp_unslash($user_login)); ?>" size="20" /></label>
			</p>
			<p>
				<label for="user_email"><?php _e( 'Correo electrónico', 'emyth' );?><br />
			...
			<p id="reg_passmail"><?php _e( 'Una contraseña será enviada a tu corro electrónico.', 'emyth' ) ?></p>
			<br class="clear" />
			<input type="hidden" name="redirect_to" value="<?php echo esc_attr( $redirect_to ); ?>" />
			<p class="submit"><input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Registrar', 'emyth');?>" /></p>
		</form>

		<p id="nav">
			<a href="<?php echo esc_url( wp_login_url() ); ?>"><?php _e( 'Acceder', 'emyth' ); ?></a> |
			<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>" title="<?php esc_attr_e( 'Contraseña perdida y encontrada', 'emyth' ) ?>"><?php _e( 'Perdiste tu contraseña?', 'emyth' );?></a> |
			<a href="<?php echo site_url('wp-login.php?action=resendec'); ?>" title="<?php esc_attr_e( 'Enviar de vuelta el enlace de confirmación', 'emyth' ); ?>"><?php _e( 'Enviar de vuelta el enlace de confirmación', 'emyth' ); ?></a>
		</p>
	...
	public function forceError( $errors, $sanitized_user_login, $user_email )
	{
		...
		if( $userLogin != 0 )
			$errors->add( 'dm_ec_userlogin_exists', __( '<strong>ERROR</strong>: Este nombre de usuario ya está registrado, elegí otro.', 'emyth' ) );

		if( $userEmail != 0 )
			$errors->add( 'dm_ec_useremail_exists', __( '<strong>ERROR</strong>: Este correo electrónico ya está registrado, elegí otro.', 'emyth' ) );
