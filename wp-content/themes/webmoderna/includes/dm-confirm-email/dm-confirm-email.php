...
class DmConfirmEmail {
	...
	public function activate() {
		...
		update_option(self::PLUGIN_ALIAS, array(
			'email_subject' => __( 'Confirmación de correo electrónico', 'emyth' ),
			'email_text' => '&lt;p&gt;' . __('Hola ', 'emyth') . ' {user_login},&lt;/p&gt;
			&lt;p&gt;   ' . __( 'Bienvenido a', 'emyth' ) . ' {site_link}.&lt;/p&gt;
			&lt;p&gt;   ' . __( 'Para confirmar tu correo electrónico, haz click en el siguiente enlace', 'emyth' ) . '&lt;/p&gt;
			&lt;p&gt;   {confirm_link}&lt;/p&gt;',
			'email_ishtml' => 'true',
			'expiry_time' => '30',
			'success_message' => __( 'Revisá tu correo electrónico y buscá el mensaje con el enlace de confirmación.', 'emyth' ),
			'user_pass_subject' => self::getPluggableUserPassSubject(),
			'user_pass_message' => self::getPluggableUserPassMessage(),
			'confirmed_message' => __( 'Tu correo electrónico está confirmado. Tu contraseña fue enviada a tu casilla de email.', 'emyth' ),
			'failed_message' => __( 'La contraseña es inválida o ha expirado.', 'emyt' ),
			'send_welcome' => 'false',
			'welcome_email_subject' => __( 'Bienvenido a nuestro sitio web.', 'emyth' ),
			'welcome_message' => '',
	...
	public static function getPluggableUserPassMessage() {
		...
		$message  = "<p>" . sprintf( __( 'Nombre de usuario: %s', 'emyth' ), '{user_login}' ) . "</p>" . "\r\n";
		$message .= "<p>" . sprintf( __( 'Contraseña: %s', 'emyth' ), '{password}' ) . "</p>" . "\r\n";
	...
	public static function getPluggableUserPassSubject() {
		...
		// see wp-includes/pluggable.php
		// function wp_new_user_notification()
		$title  = '{site_title}' . __( 'Tu nombre de usuario y contraseña', 'emyth' );
